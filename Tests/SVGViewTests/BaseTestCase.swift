...
class BaseTestCase: XCTestCase {
    ...
    func compareToReference(_ fileName: String) {
        ...
        let svgURL = bundle.url(
            forResource: fileName, withExtension: "svg", subdirectory: "w3c/\(dir)/svg/")!
        let refURL = bundle.url(
            forResource: fileName, withExtension: "ref", subdirectory: "w3c/\(dir)/refs/")!
    ...
    func compareResultStrings(nodeContent: String?, referenceContent: String?) {
        ...
        if nodeContent != referenceContent {
            XCTFail(
                "nodeContent is not equal to referenceContent. "
                    + prettyFirstDifferenceBetweenStrings(s1: nodeContent, s2: referenceContent))
        }
    }

    func prettyFirstDifferenceBetweenStrings(s1: String, s2: String) -> String {
        return prettyFirstDifferenceBetweenNSStrings(s1: s1 as NSString, s2: s2 as NSString)
            as String
    }
}
...
private func prettyFirstDifferenceBetweenNSStrings(s1: NSString, s2: NSString) -> NSString {
    let firstDifferenceResult = firstDifferenceBetweenStrings(s1: s1, s2: s2)
    return prettyDescriptionOfFirstDifferenceResult(
        firstDifferenceResult: firstDifferenceResult, s1: s1, s2: s2)
}
...
private func prettyDescriptionOfFirstDifferenceResult(
    firstDifferenceResult: FirstDifferenceResult, s1: NSString, s2: NSString
) -> NSString {
    ...
    func diffString(index: Int, s1: NSString, s2: NSString) -> NSString {
        ...
        func windowSubstring(s: NSString, range: NSRange) -> String {
            let validRange = NSMakeRange(
                range.location, min(range.length, s.length - range.location))
